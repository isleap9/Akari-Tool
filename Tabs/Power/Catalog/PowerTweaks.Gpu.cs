...
{
    // MVVM PORT: extracted verbatim from net8 PowerTab.Gpu.cs.
    // Section "GPU Power". Each row is gated on its own probe; option labels are parsed
    // from the driver's own powercfg /q output (a driver exposing extra indices self-sizes
    // the row). Returns only the rows that survive gating — an empty array when the machine
    // exposes no vendor GPU power subgroups (net8 skipped the section header in that case).
    public static partial class PowerTweaks
    {
        ...
        public static TweakDefinition[] GpuPower(Action<string> Log)
        {
            ...
            if (present.Length == 0)
            {
                Log("Power: no vendor GPU power subgroups on this system — GPU Power section skipped.");
            ...
            foreach (var row in present)
            {
                ...
                {
                    Log($"Power: {row.Name} exposed no possible settings — row skipped.");
                    continue;
                }
