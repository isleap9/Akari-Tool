...
namespace AkariTool.Tabs
{
    ...
    {
        // ── EXPLORER ▸ THIS PC FOLDERS ──
        private void BuildExplorerThisPc(StackPanel panel)
        {
            // ── This PC Folders ───────────────────────────────────────────────
            var thisPcSection = TweakHelpers.BuildSection(panel, "This PC Folders");

            foreach (var (id, title, desc, guid) in ThisPcFolders)
            {
                var capturedGuid  = guid;
                var capturedTitle = title;
                TweakHelpers.AddTweakRow(thisPcSection, new TweakDefinition
                {
                    Id          = id,
                    Name        = title,
                    Description = desc,
                    ...
                    Apply       = enable => WriteExplorer(() =>
                    {
                        ...
                        Service?.Log($"[EXPLORER] {capturedTitle} {(enable ? "shown" : "hidden")}.");
                    }),
                });
            }
            ...
            TweakHelpers.AddTweakRow(thisPcSection, new TweakDefinition
            {
                ...
                Apply       = enable => WriteExplorer(() =>
                {
                    ...
                    Service?.Log($"[EXPLORER] 3D Objects {(enable ? "shown" : "hidden")}.");
                }),
            });
            ...
            TweakHelpers.AddTweakRow(thisPcSection, new TweakDefinition
            {
                ...
                Apply       = enable => WriteExplorer(() =>
                {
                    ...
                    Service?.Log($"[EXPLORER] Duplicate removable drives {(enable ? "hidden" : "restored")}.");
                }),
            });
        }
