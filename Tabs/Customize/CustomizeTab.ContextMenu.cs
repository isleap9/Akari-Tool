...
namespace AkariTool.Tabs
{
    public partial class CustomizeTab
    {
        ...
        private void BuildContextMenu(StackPanel panel)
        {
            panel.Children.Add(PageHeader("Context Menu", "Add or remove entries from the Windows right-click context menu.",
                withActions: true, panel));

            // Registry-based context menu entries
            // Every entry is "add a shell verb / association that Windows does not ship
            // with", so the Windows factory state is always absent → DefaultState = false.
            // None of these are Winhance settings, and whether an entry is desirable is a
            // matter of taste, so RecommendedState is deliberately left unset.
            ...
            var entriesSection = TweakHelpers.BuildSection(panel, "Entries");
            ...
            foreach (var (id, title, description, add, remove, detect) in entries)
            {
                ...
                TweakHelpers.AddTweakRow(entriesSection, new TweakDefinition
                {
                    Id           = id,
                    Name         = title,
                    ...
                    Apply        = enable =>
                    {
                        try
                        {
                            ...
                            Service?.Log($"[CONTEXT] {capturedTitle} {(enable ? "added" : "removed")}.");
                        }
                        catch (Exception ex) { Service?.Log($"[CONTEXT] ERROR {capturedTitle}: {ex.Message}"); }
                    },
                });
            }
        }
