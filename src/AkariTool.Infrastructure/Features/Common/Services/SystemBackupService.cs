...
public class SystemBackupService : ISystemBackupService
{
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            var restorePointName = name
                ?? $"Akari Tool Restore Point - {DateTime.Now.ToString("yyyy-MM-dd HH:mm", CultureInfo.InvariantCulture)}";
    ...
    /// Returns (-1, -1) if parsing fails. Internal for test coverage (ParseTag precedent).
    /// </summary>
    internal static (long UsedBytes, long MaxBytes) ParseShadowStorageOutput(string output)
    {
        long usedBytes = -1;
        long maxBytes = -1;
    ...
    /// Internal for test coverage (ParseTag precedent).
    /// </summary>
    internal static long ParseByteValue(string line)
    {
