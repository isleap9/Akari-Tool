...
/// <summary>
/// Applies PowerCfg settings by writing the AC/DC value index into the ACTIVE power
/// scheme via PowrProf P/Invoke, then re-activating the scheme. Ported 1:1 from
/// Winhance's PowerCfgApplier (Winhance uses the same active-scheme semantics — Akari
/// previously used a persistent custom "Akari Performance" scheme; per isleap's
/// decision the declarative Power tab writes to whatever plan is active).
/// </summary>
public sealed class PowerCfgApplier(
    IPowerSettingsQueryService powerSettingsQueryService,
    IHardwareDetectionService hardwareDetectionService,
    IComboBoxResolver comboBoxResolver,
    IAkariLogService logService) : IPowerCfgApplier
{
    public async Task ApplyPowerCfgSettingsAsync(SettingDefinition setting, bool enable, object? value)
    {
        ...
        else
        {
            ...
            {
                logService.Log(LogLevel.Info, $"[PowerCfgApplier] Skipping PowerCfg setting '{setting.Id}' - no value provided (old config format)");
                return;
            }
    ...
    private int ConvertToSystemUnits(int displayValue, string? units)
    {
        return units?.ToLowerInvariant() switch
        {
            ...
            // Mirror of Winhance: "Milliseconds" is the only setting where the
            // system unit and display unit match 1:1. Was `/ 1000`, which silently
            // divided every USB suspend timeout write by 1000.
