...
/// <summary>
/// Service for building tooltip data for settings.
/// Reads registry, scheduled tasks, and powercfg values for the Technical Details panel.
/// Ported from Winhance's TooltipDataService.
/// </summary>
public sealed class TooltipDataService : ITooltipDataService
{
        ...
        IPowerSettingsQueryService powerSettingsQueryService)
    {
        ...
        _powerSettingsQueryService = powerSettingsQueryService ?? throw new ArgumentNullException(nameof(powerSettingsQueryService));
    }
    ...
    public async Task<IReadOnlyDictionary<string, SettingTooltipData>> GetTooltipDataAsync(IEnumerable<SettingDefinition> settings)
    {
        ...
        {
            _logService.Log(LogLevel.Warning, $"[TooltipDataService] Error fetching bulk tooltip data: {ex.Message}");
        }
    ...
    public async Task<SettingTooltipData?> RefreshTooltipDataAsync(string settingId, SettingDefinition setting)
    {
        ...
        {
            _logService.Log(LogLevel.Warning, $"[TooltipDataService] Error refreshing tooltip for '{settingId}': {ex.Message}");
            return null;
        }
    }
    ...
    public async Task<IReadOnlyDictionary<string, SettingTooltipData>> RefreshMultipleTooltipDataAsync(IEnumerable<SettingDefinition> settings)
    {
        ...
        {
            _logService.Log(LogLevel.Warning, $"[TooltipDataService] Error refreshing multiple tooltips: {ex.Message}");
        }
    ...
    private async Task<SettingTooltipData?> GetTooltipDataForSettingAsync(SettingDefinition setting)
    {
        ...
        try
        {
            ...
            if (hasRegistrySettings)
            {
                ...
                foreach (var registrySetting in registrySettings)
                {
                    ...
                    {
                        _logService.Log(LogLevel.Debug, $"[TooltipDataService] Error reading registry for tooltip '{registrySetting.KeyPath}\\{registrySetting.ValueName}': {ex.Message}");
        ...
        {
            _logService.Log(LogLevel.Warning, $"[TooltipDataService] Error building tooltip data for '{setting.Id}': {ex.Message}");
            return null;
        }
    }
